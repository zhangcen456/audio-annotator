...
StageThreeView.prototype = {
    ...
    updateTagContents: function(proximityTags, annotationTags, wavText) {
    ...
    createAnnotationTags: function(annotationTags) {
        ...
        // console.log(annotationTags);//两个tag的list？
        ...
        var textbox=$('<input>',{class:'annotation_input'});
        annotationContainer.append(textbox);
        var textbtn=$('<button>',{class:'annotation_tag btn disabled',text:'annotate'});
        textbtn.click(function () {
            // this.currentRegion.annotation=[]
            var content=$('input.annotation_input').val()
            $(my).trigger('change-tag', [{annotation: content}]);
            $('input.annotation_input').val('')
        })
        annotationContainer.append(textbtn);

        var ratebtn1=$('<button>',{class:'audio_rate btn',text:'0.75x'});
        ratebtn1.click(function(){
            $(my).trigger('change-rate', [0.75]);
        })
        var ratebtn2=$('<button>',{class:'audio_rate btn',text:'1.0x'});
        ratebtn2.click(function(){
            $(my).trigger('change-rate', [1.0]);
        })

        return annotation.append([annotationLabel, annotationContainer,ratebtn1,ratebtn2]);
    },
...
AnnotationStages.prototype = {
    ...
    reset: function(proximityTags, annotationTags, solution, alwaysShowTags, wavText) {
        this.clear();
        // Update all Tags' Contents
        this.alwaysShowTags = alwaysShowTags || false;
        this.updateContentsTags(proximityTags, annotationTags, wavText);
    ...
    updateContentsTags: function(proximityTags, annotationTags, wavText) {
